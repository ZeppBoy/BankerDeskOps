...
public partial class BankClientsViewModel : ObservableObject
{
    ...
    [ObservableProperty] private ObservableCollection<BankClientDto> clients = new();
    [ObservableProperty] private bool isLoading;
    [ObservableProperty] private string? errorMessage;
    [ObservableProperty] private BankClientDto? selectedClient;
    ...
    [ObservableProperty] private DateTime dateOfBirth = new(1990, 1, 1);
    ...
    {
        if (SelectedClient == null) { ErrorMessage = "Please select a client to update"; return; }
    ...
    {
        if (SelectedClient == null) { ErrorMessage = "Please select a client to suspend"; return; }
        if (SelectedClient.Status == ClientStatus.Suspended) { ErrorMessage = "Client is already suspended"; return; }
        ...
        try
        {
            ...
            if (updated != null) { ReplaceInList(updated); ErrorMessage = "Client suspended successfully"; }
        }
        catch (Exception ex)
        {
    ...
    {
        if (SelectedClient == null) { ErrorMessage = "Please select a client to activate"; return; }
        if (SelectedClient.Status == ClientStatus.Active) { ErrorMessage = "Client is already active"; return; }
        ...
        try
        {
            ...
            if (updated != null) { ReplaceInList(updated); ErrorMessage = "Client activated successfully"; }
        }
        catch (Exception ex)
        {
    ...
    {
        if (SelectedClient == null) { ErrorMessage = "Please select a client to delete"; return; }
    ...
    /// <summary>
    /// Auto-populates the edit form when a client is selected in the grid.
    /// </summary>
    ...
    private CreateBankClientRequest BuildRequest() => new()
    {
        FirstName   = FirstName,
        LastName    = LastName,
        Email       = Email,
    ...
    private void ReplaceInList(BankClientDto updated)
    {
