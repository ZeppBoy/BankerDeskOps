...
{
    private readonly GrpcRetailAccountApiService _accountApiService;
    private readonly ILogger<RetailAccountsViewModel> _logger;

    [ObservableProperty] private ObservableCollection<RetailAccountDto> accounts = new();
    [ObservableProperty] private bool isLoading;
    [ObservableProperty] private string? errorMessage;
    [ObservableProperty] private string customerName = string.Empty;
    [ObservableProperty] private AccountType accountType = AccountType.Checking;
    [ObservableProperty] private decimal initialDeposit;
    [ObservableProperty] private decimal transactionAmount;
    [ObservableProperty] private RetailAccountDto? selectedAccount;

    public RetailAccountsViewModel(GrpcRetailAccountApiService accountApiService, ILogger<RetailAccountsViewModel> logger)
    {
    ...
    {
        if (string.IsNullOrWhiteSpace(CustomerName)) { ErrorMessage = "Customer name is required"; return; }
        ...
        try
        {
            ...
            var created = await _accountApiService.OpenAccountAsync(request);
    ...
    {
        if (SelectedAccount == null)       { ErrorMessage = "Please select an account"; return; }
        if (TransactionAmount <= 0)        { ErrorMessage = "Deposit amount must be greater than zero"; return; }
        ...
        try
        {
            ...
            var updated = await _accountApiService.DepositAsync(SelectedAccount.Id, request);
            if (updated != null)
            {
    ...
    {
        if (SelectedAccount == null)       { ErrorMessage = "Please select an account"; return; }
        if (TransactionAmount <= 0)        { ErrorMessage = "Withdrawal amount must be greater than zero"; return; }
        ...
        try
        {
            ...
            var updated = await _accountApiService.WithdrawAsync(SelectedAccount.Id, request);
            if (updated != null)
            {
                ReplaceInList(updated);
                TransactionAmount = 0;
    ...
    {
        if (SelectedAccount == null) { ErrorMessage = "Please select an account to close"; return; }
    ...
    private void ReplaceInList(RetailAccountDto updated)
    {
